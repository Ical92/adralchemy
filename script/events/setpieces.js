...
Events.Setpieces = {
	...
	"hut": { /* Clearing the Alchemy Hut */
		title: _('An Alchemists Hut'),
		scenes: {
			'start': {
				text: [
					_('an abandoned hut stands above the land, scaffolding collapsing.'),
					_('an unfamiliar scent lingers in the air.'),
				],
				notification: _('the door of the hut loosely hangs open'),
				buttons: {
					'enter': {
						text: _('go inside'),
						nextScene: { 1: 'enter' }
			...
			'enter': {
				...
				notification: _('an alchemist lurches foward, afflicted by their own creation'),
				buttons: {
					'leave': {
						text: _('leave'),
						cooldown: Events._LEAVE_COOLDOWN,
			...
			'cleared': {
				text: [
					_('the alchemist is dead.'),
					_('ingredients and faded formulas are scattered on the floor.')
				],
				notification: _('the hut is clear'),
				onLoad: function() {
					World.drawRoad();
					World.state.alchemyhut = true;
					World.markVisited(World.curPos[0], World.curPos[1]);
				},
				buttons: {
					'leave': {
