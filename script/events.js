...
var Events = {
	...
	startCombat: function(scene) {
		...
		*/
		...
		if((Path.outfit['energy flask'] || 0) !== 0) {
			Events.createUseEnergyButton().appendTo(healBtns);
		}
		if((Path.outfit['vigor flask'] || 0) !== 0) {
			Events.createUseVigorButton().appendTo(healBtns);
		}
	...
	createUseEnergyButton: function(cooldown) {
		if (cooldown == null) {
			cooldown = Events._MEDS_COOLDOWN;
		}
	...
	createUseVigorButton: function(cooldown) {
		if (cooldown == null) {
			cooldown = Events._MEDS_COOLDOWN;
		}
	...
	useEnergy: function(btn) {
		Events.doHeal('energy flask', World.energyHeal(), btn);
		AudioEngine.playSound(AudioLibrary.USE_MEDS);
	},

	useVigor: function(btn) {
		if(Path.outfit['vigor flask'] > 0) {
			(dmgBonus += 1);
			Events.drawFloatText('bonus ' + dmgBonus, '#wanderer .hp');
			Path.outfit['vigor flask']--;
			World.updateSupplies();
			if(Path.outfit['vigor flask'] === 0) {
				Button.setDisabled(btn, true);
			}
